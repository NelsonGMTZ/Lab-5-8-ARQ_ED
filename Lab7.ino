...
const int PIN_POT = A0; // entrada analogica del potenciometro
Servo miServo;
bool modoAuto = true;
...
void loop() {
 ...
 char c = Serial.read();
 if (c == '\n') { procesarComando(bufferCmd); bufferCmd = ""; }

 else if (c != '\r') bufferCmd += c;
 }
 if (modoAuto) {
 // Demo: rampa de brillo y movimiento de servo
 static int val = 0;
 static int dir = 1;
 ...
 // Modo manual: potenciometro controla servo y LED
...
void procesarComando(String cmd) {
 cmd.trim();
 if (cmd == "AUTO") { modoAuto = true; Serial.println("[OK] Modo automatico"); }
 else if (cmd == "POT") { modoAuto = false; Serial.println("[OK] Modo potenciometro"); }
